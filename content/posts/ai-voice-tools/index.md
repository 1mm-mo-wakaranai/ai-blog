...
| 目的 | おすすめツール | 理由 |
|------|-------------|------|
| 会議の文字起こし | Notta / otter.ai | リアルタイム文字起こし対応 |
| 録音した音声の文字起こし | Whisper（OpenAI） | 精度が高い、日本語対応 |
| 文字起こし+要約 | ChatGPT（音声入力） | 文字起こしと要約が1ツールで完結 |
| テキスト→音声（読み上げ） | VOICEVOX / ElevenLabs | 自然な音声、キャラクター音声対応 |

### 導入前に確認すべきこと

- **録音の許可**: 会議を録音する場合は参加者の同意を得る
- **データの保存先**: ツールによっては音声データがクラウドに保存される。機密会議は注意
- **精度の限界**: 固有名詞や専門用語は間違えやすい。重要な箇所は手動で確認


### 今日から試せるアクション

1. 次の会議を（許可を取って）スマホで録音する
2. 録音ファイルをWhisper（またはNotta等）で文字起こしする
3. 文字起こし結果をChatGPTに「箇条書きで要約して」と依頼する
4. 従来の議事録作成時間と比較してみる


### 文字起こしの精度を上げるコツ

- **マイクを近づける**: スマホ録音なら発言者から50cm以内に
- **雑音を減らす**: エアコンの風が直接当たらない場所で録音
- **話者を分ける**: 「Aさん：」「Bさん：」の分離は手動で補正する前提で
- **固有名詞リスト**: 事前にツールに登録しておくと認識精度UP（Notta等で対応）
...
|---------|--------|---------|
| 録音 | スマホの録音アプリ | リアルタイム |
| 文字起こし | Whisper / Notta | 音声の1/10の時間 |
| 要約 | ChatGPT / Claude | 30秒 |
