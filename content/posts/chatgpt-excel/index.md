...
![ChatGPT×Excel活用 理解度の変化](images/comparison-before-after.png)
...
## 実際にChatGPTでExcel作業を効率化してみた！（筆者の体験）

筆者がExcelの面倒な作業をChatGPTに手伝ってもらった結果です。

### VLOOKUP関数の生成

「A列にある商品コードを元に、別シートの価格表から価格を引っ張ってくる数式を教えて」と聞いたら、VLOOKUPの数式を正確に生成してくれました。さらに「XLOOKUPとの違いも教えて」と追加質問したら、分かりやすい比較表付きで回答。

### マクロの生成

「毎週月曜日に先週のデータをバックアップシートにコピーするマクロを書いて」→ VBAコードが一発で生成。そのままExcelに貼り付けて動作しました。

### 使い方のコツ

- **具体的にデータ構造を伝える**: 「A列に日付、B列に商品名、C列に売上金額があります」
- **やりたいことを日本語で説明するだけでOK**: 数式の知識がなくても問題ない
- **生成された数式はテストデータで検証する**: AIが間違えることもあるため

Excelの関数やマクロが苦手な人ほどChatGPTの恩恵を受けます。「やりたいこと」を日本語で伝えるだけで数式が出てくるので、VLOOKUPの書き方を暗記する必要はもうありません。Excel作業で詰まったら、まずChatGPTに聞いてみてください。


### よくある失敗パターンと対処法

**失敗1**: ChatGPTが生成した数式をそのまま貼ったらエラーが出た
- **原因**: 日本語版Excelではセミコロン（;）が区切りだが、ChatGPTはカンマ（,）で生成する場合がある
- **対処**: 「日本語版Excel用の数式を書いて」と明示する

**失敗2**: VLOOKUP関数を教えてもらったが古い書き方だった
- **原因**: ChatGPTの学習データに古い情報が含まれている
- **対処**: 「XLOOKUP（新しい方）で書いて」と指定する。Excel 365ならXLOOKUP推奨

**失敗3**: マクロが動かなかった
- **原因**: セキュリティ設定でマクロが無効になっていた
- **対処**: ファイル→オプション→セキュリティセンターでマクロを有効化する


### 今日から試せるアクション

1. Excelで「面倒だな」と思っている作業を1つ思い出す
2. ChatGPTに「Excelで○○をしたい。データはA列に○○、B列に○○があります」と具体的に説明
3. 生成された数式をテストデータで検証する
4. うまくいったら実データに適用する


### すぐ使えるExcel質問テンプレート

ChatGPTにExcelの質問をするときは、以下の形式で聞くと正確な回答が返ってきます：

```
Excelで以下の処理をしたいです。数式を教えてください。
- データ: A列に[データ内容]、B列に[データ内容]
- やりたいこと: [具体的な処理]
- 条件: [あれば]
- Excel バージョン: [365/2019等]
- 日本語版を使っています
```

**使用例:**
「A列に日付（2026/1/1〜）、B列に売上金額があります。月ごとの合計をC列に出したいです。Excel 365の日本語版です」

→ SUMIFS関数やピボットテーブルの作り方を正確に教えてくれます。


### Excel × ChatGPTの黄金パターン3つ

1. **「○○する数式を教えて」** → VLOOKUP/SUMIFS等の数式生成
2. **「このデータをグラフにするならどんな形式がいい？」** → 可視化のアドバイス
3. **「毎週やるこの作業を自動化するマクロを書いて」** → VBAコード生成

この3パターンだけでExcel作業の70%はAIに手伝ってもらえます。
